#定义常量

#直接设置成长度为 256的一维数组，就不需要拆分 8 比特确定行和列了
s_box = []

FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC]
CK = [0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
              0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
              0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
              0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279]

def left_rotate(x, n): #左移
    return ((x << n) & 0xFFFFFFFF) | (x >> (32 - n))
# S-box 
def sm4_sbox(x):
    return (s_box[x >> 24] << 24) | (s_box[(x >> 16) & 0xFF] << 16) | (s_box[(x >> 16) & 0xFF] << 8) | s_box[x & 0xFF]

#移位
def sm4_t(x):
    return x ^ left_rotate(x, 2) ^ left_rotate(x, 10) ^ left_rotate(x, 18) ^ left_rotate(x, 24)

def convert_to_integers(plaintext):
    # 将16进制字符串转换为32比特整数表示
    integers = [int(plaintext[i:i+8], 16) for i in range(0, len(plaintext), 8)]
    return integers
def convert_to_hex(integers):
    # 将32比特整数转换为16进制字符串表示
    hex_strings = [format(integer, '08x') for integer in integers]
    return ''.join(hex_strings)

def sm4_key_schedule(key):
    #生成子密钥
    rk = [0] * 36
    for i in range(4):
        rk[i] = key[i] ^ FK[i]
    for i in range(32):
        y=sm4_sbox(rk[i + 1] ^ rk[i + 2] ^ rk[i + 3] ^ CK[i])
        rk[i + 4] = rk[i] ^ y^left_rotate(y, 13)^left_rotate(y, 23)
    rk=rk[4:]
    return rk


def sm4_encrypt_decrypt(plaintext, rk):
    # 对单个 128 位数据块（16 字节)加密
    x0, x1, x2, x3 = plaintext
    for i in range(32):
        tmp = x1 ^ x2 ^ x3 ^ rk[i]
        tmp = sm4_sbox(tmp)
        tmp = sm4_t(tmp)
        x0 ^= tmp
        x0, x1, x2, x3 = x1, x2, x3, x0

    return [x3, x2, x1, x0]  #反序变换
def sm4_encrypt(plaintext, key):
   
    # 生成子密钥
    rk = sm4_key_schedule(key)
    # 加密
    encrypted = sm4_encrypt_decrypt(plaintext, rk)
    return encrypted

def sm4_decrypt(ciphertext, key):
   
    # 生成子密钥
    rk = sm4_key_schedule(key)
    temp=[0]*32
    for i in range(32):
        temp[i]=rk[i]
    for i in range(32):
        rk[i]=temp[31-i]
    # 解密
    decrypted = sm4_encrypt_decrypt(ciphertext, rk)
    return decrypted

#运行测试
if __name__ == "__main__":
    plaintext_hex = "0123456789abcdeffedcba987654321"
    key="0123456789abcdeffedcba987654321"
    print("要加密的原文为：",plaintext_hex)
    plaintext_integers = convert_to_integers(plaintext_hex)
    key=convert_to_integers(key)
    # 加密
    ciphertext = sm4_encrypt(plaintext_integers, key)
    ciphertext=convert_to_hex(ciphertext)
    print("密文:", ciphertext)
    #解密
    ciphertext_integers = convert_to_integers(ciphertext)
    plaintext = sm4_decrypt(ciphertext_integers, key)
    plaintext=convert_to_hex(plaintext)
    print("明文:",  plaintext)
